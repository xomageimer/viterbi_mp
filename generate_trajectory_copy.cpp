...
int main(int argc, char ** argv) {
    ...
    std::vector<std::vector<size_t>> prev {std::vector<size_t>(json_graph[0]["particles"]["x"].size(), -1)};
    ...
    size_t threads_count = std::max(static_cast<unsigned int>(2), std::thread::hardware_concurrency());
    auto block_count = total_steps / threads_count;
    size_t block_start = 0;
    size_t block_end = block_start + block_count;
